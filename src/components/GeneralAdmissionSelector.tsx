...
export const GeneralAdmissionSelector: React.FC<GeneralAdmissionSelectorProps> = ({ tiers }) => {
  const { gaQuantities, setGaQuantity, inlineError } = useBooking();
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="space-y-4">
        {tiers.map((tier) => {
          const count = gaQuantities[tier.id] || 0;
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  {isSelected && (
                    <motion.span
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="inline-flex items-center gap-1 rounded-full bg-theme-accent/15 px-2 py-0.5 text-[10px] font-mono text-theme-accent"
                    >
              ...
              <div className="flex items-center self-end sm:self-center gap-3 bg-theme-bg/60 p-1.5 rounded-xl border border-theme-border">
                <motion.button
                  ...
                  onClick={() => setGaQuantity(tier.id, count - 1)}
                  disabled={count <= 0}
